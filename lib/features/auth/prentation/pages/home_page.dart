...
class _HomePageState extends State<HomePage>{
    ...
    return
      Scaffold(
        appBar: AppBar(
          ...
          backgroundColor: AppPallete.backgroundColor, // Theme color for CarbonCourier
        ),
        body: Column(
          children: [
            Padding(
                        ...
                        children: [
                          DecoratedBox(
                            ...
                            child: Padding(
                              ...
                              child: Text("Estimated emissions: $_estimatedEmissions kg CO2", style: const TextStyle(fontSize: 18)),
                            )
                          ),
                        ],
                      ),
  ...
  Future<void> _getRoute() async {
    ...
    try {
      final url = 'https://maps.googleapis.com/maps/api/directions/json?origin=$from&destination=$to&key=${Config.googleApiKey}';
      final response = await http.get(Uri.parse(url));
      final data = json.decode(response.body);
      final distance = data["routes"][0]["legs"][0]["distance"]["value"];
      
      final url2 = 'http://10.0.2.2:5000/predict-emmisions?vehicle-class=Cargo Van&engine-size=4&fuel-type=Z&fuel-consumption=12&weight=2000&meters=$distance';
      ...
      if (data['status'] == 'OK')  {
        ...
        setState(() {
          ...
          _estimatedEmissions = (emissionsData["total_emissions"] / 1000).toStringAsFixed(1);
        });
